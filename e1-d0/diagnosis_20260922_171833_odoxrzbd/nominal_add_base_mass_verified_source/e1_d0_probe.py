...
def run(args, out, report):
    ...
    if args.mode == 'nominal':
        ...
        # scene.reset clears buffers, but does not restore articulation states.
        # Retain deterministic state-writing events, with degenerate ranges.
        cfg.events.reset_base = copy.deepcopy(original_events.reset_base)
        cfg.events.reset_base.params['pose_range'] = {}
        cfg.events.reset_base.params['velocity_range'] = {}
        cfg.events.reset_robot_joints = copy.deepcopy(original_events.reset_robot_joints)
        ...
        if args.restore in ('joint_pos', 'joint_vel'):
            cfg.events.reset_robot_joints = original_events.reset_robot_joints
            cfg.events.reset_robot_joints.params['velocity_range' if args.restore=='joint_pos' else 'position_range'] = (0.,0.) if args.restore=='joint_pos' else (1.,1.)
        elif args.restore == 'root_vel':
            cfg.events.reset_base = original_events.reset_base
            cfg.events.reset_base.params['pose_range'] = {}
        elif args.restore in ('physics_material','add_base_mass'):
            setattr(cfg.events,args.restore,getattr(original_events,args.restore))
    ...
    if args.mode=='nominal':
        checks=dict(default_joint_pos=bool(torch.equal(robot.data.joint_pos,robot.data.default_joint_pos)),
                    zero_joint_vel=bool((robot.data.joint_vel==0).all()),
                    zero_root_vel=bool((robot.data.root_state_w[:,7:]==0).all()),
                    default_root_height=bool(torch.equal(robot.data.root_pos_w[:,2],robot.data.default_root_state[:,2])),
                    zero_command=bool((cmd.command==0).all()),observation_noise_disabled=not cfg.observations.policy.enable_corruption)
        report['nominal_initial_state_checks']=checks
        assert all(v for k,v in checks.items() if k!={'joint_pos':'default_joint_pos','joint_vel':'zero_joint_vel','root_vel':'zero_root_vel'}.get(args.restore)),checks
    ...
    report['continue_after_termination'] = args.continue_after_termination
    report['restored_factor'] = args.restore
    completed = set()
    control_step = 0
    physics_step = 0
    ...
    pair_views = []
    filtered_file = (out/'filtered_contacts.jsonl').open('w')
    report['pair_view_net_max_error'] = 0.0
    if args.pair_forces:
        for i in range(raw.num_envs):
            names = ['left_wrist_yaw_link','right_wrist_yaw_link','left_hip_roll_link','right_hip_roll_link',
                     'left_ankle_roll_link','right_ankle_roll_link']
            filters = [f'/World/envs/env_{i}/Robot/{n}' for n in robot.body_names]+['/World/ground/*']
            for name in names:
                view = sensor._physics_sim_view.create_rigid_contact_view(f'/World/envs/env_{i}/Robot/{name}',
                    filter_patterns=filters,max_contact_data_count=256)
    ...
    def update_hook(dt):
        ...
        for i,name,filters,view in pair_views:
            ...
            report['pair_view_net_max_error'] = max(report['pair_view_net_max_error'],float((net-sensor.data.net_forces_w[i,sensor.body_names.index(name)]).abs().max()))
            matrix = view.get_contact_force_matrix(dt=raw.physics_dt)[0]
            hits = (matrix.norm(dim=-1)>0).nonzero().flatten().tolist()
            if hits:
                force,point,normal,separation,count,start = view.get_contact_data(dt=raw.physics_dt)
                for j in hits:
                    n,offset = int(count[0,j]),int(start[0,j])
                    filtered_file.write(json.dumps(dict(episode_id=i,control_step=control_step,physics_step=physics_step,
                        time=physics_step*raw.physics_dt,body=name,other=filters[j],force=plain(matrix[j]),
                        contact_forces=plain(force[offset:offset+n]),points=plain(point[offset:offset+n]),
                        normals=plain(normal[offset:offset+n]),separations=plain(separation[offset:offset+n])))+'\n')
        for i in range(raw.num_envs):
            if not active[i]:
                continue
            row = state_row(i)
    ...
    def compute_hook(*a, **kw):
        ...
        for i in range(raw.num_envs):
            ...
            if i not in completed and (row['terminated'] or row['truncated'] or control_step>=100):
                ...
                completed.add(i)
                if not args.continue_after_termination:
                    active[i] = False
        return reward
    ...
    try:
        ...
        report['final_root_state'] = plain(robot.data.root_state_w)
        ...
        subscription = None
        traces.close(); control.close(); contacts.close(); filtered_file.close()
        env.close()
...
def main():
    ...
    parser.add_argument('--pair-forces', action='store_true')
    parser.add_argument('--continue-after-termination', action='store_true')
    parser.add_argument('--restore', choices=['none','joint_pos','joint_vel','root_vel','physics_material','add_base_mass'],default='none')
