...
def run(args, out, report):
    ...
    # OnPolicyRunner resets env during construction. Construct it for zero as
    # well so all three controllers get the exact same seeded reset sequence.
    ...
    assert ground_paths, "Ground collision prim must exist"
    ...
    for i in range(count):
        filters = [f"/World/envs/env_{i}/Robot/{n}" for n in robot.body_names] + ground_paths
        ...
        for i, name, filters, view in pair_views:
            ...
            try:
                ...
                for j in hits:
                    ...
                    category = "robot_ground_contact" if j >= len(robot.body_names) else "robot_robot_self_contact"
    ...
    try:
        with torch.inference_mode():
            if args.mode == "ground_positive":
                ...
                # Plane pairs may not be exposed by GPU filtered contact views.
                # Preserve observed ground support separately from object-pair
                # confirmation; never promote indirect evidence into a pair.
                trace.flush()
                final_geometry = json.loads((out / "physics_trace.jsonl").read_text().splitlines()[-1])
                for name in ("torso_link", "pelvis"):
                    j = sensor.body_names.index(name)
                    minimum = min(s["min_z"] for s in final_geometry["geometry"] if s["body"] == name)
                    force = float(sensor.data.net_forces_w[0, j].norm())
                    if abs(minimum) < 1e-3 and force > 1:
                        add_contact(0, name, "ground_collision_plane_object_unconfirmed", force,
                                    [], "unconfirmed", "support inference: collision geometry at z=0 + sensor force",
                                    extra={"geometry_min_z_m": minimum, "ground_prim_candidate": ground_paths})
                ...
                report["ground_positive_indirect_support"] = {
                    n: any(r["link_a"] == n and r["evidence"].startswith("support inference") for r in contacts)
                    for n in ("torso_link", "pelvis")}
            else:
